...
from db_postgres.crud.reminders import update_reminders_status, add_reminders, status_search
import time
import asyncio
...
async def main():
    ...
    await add_reminders(user_id)
    x = await update_reminders_status(user_id, 0)
    print(x.user_id)
    x = await status_search(user_id)
    print(x.status)
